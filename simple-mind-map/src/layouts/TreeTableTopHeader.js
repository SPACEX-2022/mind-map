import {asyncRun, getNodeIndexInNodeList, walk} from "../utils";
import {CONSTANTS} from "../constants/constant";
import Base from "./Base";

class TreeTableTopHeader extends Base {
    ...
    computedBaseValue() {
        ...
        walk(
            ...
            (cur, parent, isRoot, layerIndex, index) => {
                ...
                if (isRoot) {
                    ...
                    if (parent._node.isRoot) {
                        newNode.left = parent._node.left
                    } else {
                        // newNode.left =
                        //     newNode.dir === CONSTANTS.LAYOUT_GROW_DIR.RIGHT
                        //         ? parent._node.left +
                    ...
                    cacheData[`layerIndex${layerIndex}MaxWidth`] = cacheData[`layerIndex${layerIndex}MaxWidth`] || 0
                    cacheData[`layerIndex${layerIndex}MaxWidth`] = Math.max(
                        cacheData[`layerIndex${layerIndex}MaxWidth`],
                        newNode.width
                    )

                    cacheData[`maxLayerIndex`] = cacheData[`maxLayerIndex`] || 0
                    cacheData[`maxLayerIndex`] = Math.max(cacheData[`maxLayerIndex`], layerIndex)
                }
                if (!cur.data.expand) {
                    return true
                }
            ...
            (cur, parent, isRoot, layerIndex) => {
                ...
                cur._node.childrenAreaHeight = len
                    ...
                    : 0;
                if (cur._node.childrenAreaHeight !== 0) {
                    cur._node.height = cur._node.childrenAreaHeight;
                }
                // console.log(cur._node.childrenAreaHeight)
                // cur._node.height = cur._node.childrenAreaHeight
            },
            true,
            0
        ...
        walk(
            ...
            (cur, parent, isRoot, layerIndex, index) => {
                if (isRoot) {
                    let width = 0;
                    for (let i = 1; i <= cacheData[`maxLayerIndex`]; i++) {
                        width += cacheData[`layerIndex${i}MaxWidth`];
                    }
                    cur._node.width = width;
                    return
                }
                cur._node.width = cacheData[`layerIndex${layerIndex}MaxWidth`];
                if (!parent._node.isRoot) {
                    cur._node.left = parent._node.left + parent._node.width
                }
            },
            null,
            true,
...
export default TreeTableTopHeader;